...
from utils.models import MLP, MLP_L2, MLP_Binary, MLP_Binary_L2 
import torch
...
'''
New code: Binary Classification Trainer



'''

def run_simulation_diffdims_binary(N_STIM, N_CTX, N_HIDDEN, N_OUT, N_RUNS, N_ITER, LRATE, SCALE_WHXS, SCALE_WHXC, SCALE_WYH, N_FACTORS):
    """runs simulations with different initial weight scales 
       and performs dimensionality analysis as well as ablation study on trained network
    ...
    for ii in range(N_FACTORS):
        ...
        for jj in range(N_RUNS):
            ...
            # initialise network and get starting values
            mlp = MLP_Binary(n_in=N_STIM, n_ctx=N_CTX, n_hidden=N_HIDDEN, n_out=N_OUT, lrate=LRATE,
                      scale_whxc=SCALE_WHXC[ii], scale_whxs=SCALE_WHXS[ii], scale_wyh=SCALE_WYH[ii])
            results['w_hxs_0'][ii, jj, :, :] = mlp.w_hxs
            results['w_hxc_0'][ii, jj, :, :] = mlp.w_hxc
            results['w_yh_0'][ii, jj, :, :] = mlp.w_yh
...
def run_simulation_norm_binary(N_STIM, N_CTX, N_HIDDEN, N_OUT, N_RUNS, N_ITER, LRATE, SCALE_WHXS, SCALE_WHXC, SCALE_WYH, N_FACTORS, LAMBDA):
    """same as above, but with L2 regulariser
...
def run_simulation_noiselevel_binary(N_STIM, N_CTX, N_HIDDEN, N_OUT, N_RUNS, N_ITER, LRATE, SCALE_WHXS, SCALE_WHXC, SCALE_WYH, SCALE_NOISE, N_FACTORS):
    """same as above, but with different levels of input noise during test phase
...
def run_simulation_difflrs_binary(N_STIM, N_CTX, N_HIDDEN, N_OUT, N_RUNS, N_ITER, LRATE, SCALE_WHXS, SCALE_WHXC, SCALE_WYH, N_FACTORS):
    """runs simulations with different initial weight scales 
       and performs dimensionality analysis as well as ablation study on trained network
